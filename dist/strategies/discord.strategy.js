...
const discord_user_schema_1 = __importDefault(require("../database/schemas/discord_user.schema"));
exports.authenticateDiscordUser = passport_1.default.use(new passport_discord_1.Strategy({
    ...
    console.log(profile);
    try {
        const discordUserDB = yield discord_user_schema_1.default.findOne({
            discordID: profile.id,
        });
        if (discordUserDB) {
            console.log(`Found user: ${discord_user_schema_1.default}`);
            return done(null, discordUserDB);
        }
        else {
            const newDiscordUser = yield discord_user_schema_1.default.create({
                discordID: profile.id,
